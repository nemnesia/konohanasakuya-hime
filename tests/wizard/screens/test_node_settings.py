...
async def test_passes_validation_when_metadata_is_json():
	# Arrange:
	screen = create(None)
	screen.accessor._https_flag.current_values = []  # pylint: disable=protected-access
	screen.accessor._domain_name.input.text = 'localhost'  # pylint: disable=protected-access
	screen.accessor._domain_name.input.buffer.validate()  # pylint: disable=protected-access
	screen.accessor._metadata_info.input.text = '{"animal": "wolf"}'  # pylint: disable=protected-access
	screen.accessor._metadata_info.input.buffer.validate()    # pylint: disable=protected-access

	# Act + Assert:
	assert screen.is_valid()


async def test_fails_validation_when_metadata_is_not_json():
	# Arrange:
	screen = create(None)
	screen.accessor._https_flag.current_values = []  # pylint: disable=protected-access
	screen.accessor._domain_name.input.text = 'localhost'  # pylint: disable=protected-access
